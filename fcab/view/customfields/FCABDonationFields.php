			...
			foreach ( $this->custom_fields as $field ):
				if ( $this->isInScope( $field, $post ) && current_user_can( $field['capability'], $post->ID ) ): ?>
                    <div class="form-field form-required fcab-custom-field-wrapper">
						<?php
						if ( $field['name'] === self::DONATION_FIELD ) {
                            ...
                            <div class="add-donation-wrapper">
                                <label for="<?php echo $donation_id; ?>">Enter new donation ($USD):</label> <input
                                        type="number" name="<?php echo $donation_id; ?>"
                                        id="<?php echo $donation_id; ?>" min="0" max="10000000" value="0"/>
                            </div>
							<?php
						} ?>
                    </div>
                    ...
                    <td class="donation-amount">
                        <div>
                            ...
                            <input type="checkbox" class="donation-delete-button"
                                   ...
                                   id="<?php echo $donation['donation_id']; ?>" checked/> <label
                                    for="<?php echo $donation['donation_id']; ?>"></label>
                        </div>
                    </td>
                </tr>
			<?php endforeach; ?>
...
//            && trim($_POST[$field_name]);
	...
	private function updateDonorDonations( array $donations ): array {
		...
		return array_filter( $donations, static function ( $donation ) use ( $active_donations ) {
